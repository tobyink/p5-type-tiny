...
Other modules using Type::Tiny in interesting ways: L<Type::Tie>, L<Scalar::Does>...
...
Specio has a somewhat nicer API, and its API is likely to improve further.
Type::Tiny's aims at complete compatbility with current versions of Moose
and Mouse, so there is a limit to how much I can deviate from the existing
