...
The C<signature_for> keyword is a wrapper around C<compile> and
C<compile_named_oo> from L<Type::Params>, depending on whether you
are using a positional or named parameter signature. Again, note that
types are expressed as strings and looked up using C<dwim_type>.
