...
At the time of writing, there is no released version of L<Attribute::Contract>
that supports Type::Tiny, but support exists in the respository.
...
As of 1.0102, L<Moose> is required to be loaded for this to work (even though
it's not actually being used for type checks). This has been reported as a
bug (L<RT#85851|https://rt.cpan.org/Ticket/Display.html?id=85851>) so will
