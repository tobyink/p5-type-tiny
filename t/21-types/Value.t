...
Basic tests for B<Value> from L<Types::Standard>.
...
isa_ok(Value, 'Type::Tiny', 'Value');
is(Value->name, 'Value', 'Value has correct name');
is(Value->display_name, 'Value', 'Value has correct display_name');
is(Value->library, 'Types::Standard', 'Value knows it is in the Types::Standard library');
ok(Types::Standard->has_type('Value'), 'Types::Standard knows it has type Value');
ok(!Value->deprecated, 'Value is not deprecated');
ok(!Value->is_anon, 'Value is not anonymous');
ok(Value->can_be_inlined, 'Value can be inlined');
is(exception { Value->inline_check(q/$xyz/) }, undef, "Inlining Value doesn't throw an exception");
ok(!Value->has_coercion, "Value doesn't have a coercion");
ok(!Value->is_parameterizable, "Value isn't parameterizable");
is(Value->type_default, undef, "Value has no type_default");
...
while (@tests) {
	...
	if ($expect eq 'xxxx') {
		...
		should_pass($value, Value, ucfirst("$label should pass Value"));
	}
	elsif ($expect eq 'fail') {
		should_fail($value, Value, ucfirst("$label should fail Value"));
	}
	else {
