...
Basic tests for B<GlobRef> from L<Types::Standard>.
...
use Test::Fatal;
use Test::TypeTiny;
use Types::Standard qw( GlobRef );

isa_ok(GlobRef, 'Type::Tiny', 'GlobRef');
is(GlobRef->name, 'GlobRef', 'GlobRef has correct name');
is(GlobRef->display_name, 'GlobRef', 'GlobRef has correct display_name');
is(GlobRef->library, 'Types::Standard', 'GlobRef knows it is in the Types::Standard library');
ok(Types::Standard->has_type('GlobRef'), 'Types::Standard knows it has type GlobRef');
ok(!GlobRef->deprecated, 'GlobRef is not deprecated');
ok(!GlobRef->is_anon, 'GlobRef is not anonymous');
ok(GlobRef->can_be_inlined, 'GlobRef can be inlined');
...
while (@tests) {
	...
	if ($expect eq 'xxxx') {
		...
		should_pass($value, GlobRef, ucfirst("$label should pass GlobRef"));
	}
	elsif ($expect eq 'fail') {
		should_fail($value, GlobRef, ucfirst("$label should fail GlobRef"));
	}
	else {
