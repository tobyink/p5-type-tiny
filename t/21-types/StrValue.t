...
Basic tests for B<StrStrValue> from L<Types::Common::StrValues>.
...
use Types::Common::Values qw( StrValue );

isa_ok(StrValue, 'Type::Tiny', 'StrValue');
is(StrValue->name, 'StrValue', 'StrValue has correct name');
is(StrValue->display_name, 'StrValue', 'StrValue has correct display_name');
is(StrValue->library, 'Types::Common::Values', 'StrValue knows it is in the Types::Common::Values library');
ok(Types::Common::Values->has_type('StrValue'), 'Types::Common::Values knows it has type StrValue');
ok(!StrValue->deprecated, 'StrValue is not deprecated');
ok(!StrValue->is_anon, 'StrValue is not anonymous');
ok(StrValue->can_be_inlined, 'StrValue can be inlined');
is(exception { StrValue->inline_check(q/$xyz/) }, undef, "Inlining StrValue doesn't throw an exception");
ok(StrValue->has_coercion, "StrValue has a coercion");
ok(!StrValue->is_parameterizable, "StrValue isn't parameterizable");
is(StrValue->type_default->(), q{}, "StrValue has a type_default");

#
# The @tests array is a list of triples:
#
# 1. Expected result - pass, fail, or xxxx (undefined).
# 2. A description of the StrValue being tested.
# 3. The StrValue being tested.
#
...
while (@tests) {
	...
	if ($expect eq 'xxxx') {
		...
		should_pass($value, StrValue, ucfirst("$label should pass StrValue"));
	}
	elsif ($expect eq 'fail') {
		should_fail($value, StrValue, ucfirst("$label should fail StrValue"));
	}
	else {
...
# Quoted numbers pass
should_pass( '0', StrValue, "Quoted 0 should pass StrValue" );
should_pass( '1', StrValue, "Quoted 1 should pass StrValue" );
should_pass( '999', StrValue, "Quoted 999 should pass StrValue" );
should_pass( '9.9', StrValue, "Quoted 9.9 should pass StrValue" );

# Coercions from the looser Str type constraint
is( StrValue->assert_coerce( 0 ), '0' );
is( StrValue->assert_coerce( 1 ), '1' );
is( StrValue->assert_coerce( 999 ), '999' );
