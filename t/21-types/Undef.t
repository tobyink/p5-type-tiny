...
Basic tests for B<Undef> from L<Types::Standard>.
...
isa_ok(Undef, 'Type::Tiny', 'Undef');
is(Undef->name, 'Undef', 'Undef has correct name');
is(Undef->display_name, 'Undef', 'Undef has correct display_name');
is(Undef->library, 'Types::Standard', 'Undef knows it is in the Types::Standard library');
ok(Types::Standard->has_type('Undef'), 'Types::Standard knows it has type Undef');
ok(!Undef->deprecated, 'Undef is not deprecated');
ok(!Undef->is_anon, 'Undef is not anonymous');
ok(Undef->can_be_inlined, 'Undef can be inlined');
...
while (@tests) {
	...
	if ($expect eq 'xxxx') {
		...
		should_pass($value, Undef, ucfirst("$label should pass Undef"));
	}
	elsif ($expect eq 'fail') {
		should_fail($value, Undef, ucfirst("$label should fail Undef"));
	}
	else {
